...
public interface QuestionVoteRepository extends CrudRepository<QuestionVote, Integer> {
	...
	@Query("SELECT qv FROM QuestionVote qv WHERE qv.question.book.id = ?1 AND qv.customer.id = ?2")
	public List<QuestionVote> findByProductAndCustomer(Integer bookId, Integer customerId);
